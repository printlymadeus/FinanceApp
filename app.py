...
st.title("🖨️ 3D Printing Business Tracker")
...
# ====================== VIEW & EDIT TRANSACTIONS ======================
    ...
    if 'edit_id' in st.session_state:
        ...
        with col1:
            ...
            new_type = st.selectbox("Type", ["Income", "Expense"], index=0 if data['type'] == "Income" else 1)
        with col2:
            if new_type == "Income":
                new_category = st.selectbox("Product", products,
                                          index=products.index(data.get('category', products[0])))
            else:
        ...
        new_amount = st.number_input("Selling Price ($)", value=float(data.get('amount') or 0))
        ...
        if st.button("💾 Save Changes", type="primary"):
            ...
            conn.execute("""UPDATE transactions SET
                         date=?, type=?, category=?, description=?, amount=?, cost=?, shipping=?,
                         customer=?, notes=? WHERE id=?""",
